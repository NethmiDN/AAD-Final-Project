...
(function(){
  if(window.BarkBuddyChatInitialized) return; // prevent duplicate
  window.BarkBuddyChatInitialized = true;
        ...
        <div class="chat-container">
          ...
          <div class="chat-panel">
            <div class="chat-panel-header">
              <div>
                <div id="chatActiveTitle" class="dog">Select a conversation</div>
  ...
  let current = { dogId:null, otherUserId:null, dogName:'', otherUsername:'' };
  ...
  async function loadConversations(){
    ...
    try {
      ...
      if(!Array.isArray(data) || data.length===0){
        convsDiv.innerHTML = `
          <div class="p-4 text-center text-muted">
            ...
            <small>Start chatting with dog owners from adoption pages</small>
          </div>`;
        return;
      }
      ...
      data.forEach((c, index)=>{
        ...
        if(current.dogId === c.dogId && current.otherUserId === c.otherUserId) item.classList.add('active');
        ...
        item.innerHTML = `
          <div class="title">
            <i class="fas fa-dog text-warning me-1"></i> 
            ${truncate(c.dogName,16)}
          </div>
          <div class="preview">${truncate(c.lastMessage,35)}</div>
          <div class="time">${displayTime}</div>`;
        
        item.onclick = ()=> openConversation(c.dogId, c.otherUserId, c.dogName, c.otherUsername);
  ...
  async function loadMessages(){
    if(!current.dogId) return;
    hideTypingIndicator();
    
    try {
      const res = await fetch(`http://localhost:8080/api/user-chat/conversation/${current.dogId}/${current.otherUserId}`, { 
  ...
  async function sendMessage(){
    ...
    if(myId === current.otherUserId){
      ...
      toast.innerHTML = `
        <div class="d-flex">
          <div class="toast-body"><i class="fas fa-info-circle me-2"></i>🐕 This is your own pet! You cannot chat with yourself.</div>
    ...
    try {
      if(imageInput.files && imageInput.files.length > 0) {
        ...
        fd.append('dogId', String(current.dogId));
        ...
        const res = await fetch('http://localhost:8080/api/user-chat/send', { 
          ...
          body: JSON.stringify({ dogId: current.dogId, receiverId: current.otherUserId, content }) 
        });
  ...
  function openConversation(dogId, otherUserId, dogName, otherUsername){
    const myId = Number(localStorage.getItem('userId'));
    if(myId === otherUserId){
    ...
    current = { dogId, otherUserId, dogName, otherUsername };
    chatActiveTitle.textContent = `${dogName}`;
    ...
    setTimeout(()=>{
      document.querySelectorAll('.chat-conv-item').forEach(i=>{
        if(i.querySelector('.title').textContent.includes(dogName)) {
  ...
  // Global function for dog cards
  window.openConversation = openConversation;

  // Enhanced function for dog cards
  window.openChat = function(dogId, ownerId, dogName){
    const myId = Number(localStorage.getItem('userId'));
    if(myId === ownerId){
      // Show friendly message instead of error
      const toast = document.createElement('div');
      ...
      toast.innerHTML = `
        <div class="d-flex">
          <div class="toast-body"><i class="fas fa-info-circle me-2"></i>This is your own pet! 🐕</div>
          <button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast"></button>
        </div>`;
      document.body.appendChild(toast);
      const bsToast = new bootstrap.Toast(toast);
    ...
    loadConversations().then(()=>{
      openConversation(dogId, ownerId, dogName, 'Owner');
    });
  };
