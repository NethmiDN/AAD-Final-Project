...
public class UserChatController {
    ...
    public ResponseEntity<?> sendMessage(@RequestHeader("Authorization") String token,
        ...
        try{
            ...
            Long dogId = Long.valueOf(body.get("dogId"));
            ...
            ChatMessage saved = chatService.sendMessage(dogId, senderId, receiverId, content, null);
    ...
    @GetMapping("/conversation/{dogId}/{otherUserId}")
    public ResponseEntity<?> getConversation(@RequestHeader("Authorization") String token,
                                             @PathVariable Long dogId,
                                             @PathVariable Long otherUserId){
        try{
            Long me = getUserId(token);
            List<ChatMessage> conversation = chatService.getConversation(dogId, me, otherUserId);
    ...
    public ResponseEntity<?> sendImage(@RequestHeader("Authorization") String token,
                                       @RequestPart("dogId") String dogIdStr,
                                       @RequestPart("receiverId") String receiverIdStr,
                                       @RequestPart(value="content", required=false) String content,
                                       @RequestPart("image") MultipartFile image){
        try {
            Long senderId = getUserId(token);
            Long dogId = Long.valueOf(dogIdStr);
            ...
            if(txt == null) txt = "";
            ChatMessage saved = chatService.sendMessage(dogId, senderId, receiverId, txt, url);
            return ResponseEntity.ok(saved);
        }catch(Exception e){
